...
export const Fabricantes = () => {
  ...
  const handleEliminarFabricante = async (fabricante) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/fabricantes/${fabricante.id_fabricante}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        ...
        const fabricantesActualizados = fabricantes.filter(
          (f) => f.id_fabricante !== fabricante.id_fabricante
        );
  ...
  const handleGuardarCambios = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/fabricantes/${fabricanteSeleccionado.id_fabricante}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        const fabricantesActualizados = fabricantes.map((f) =>
          f.id_fabricante === fabricanteSeleccionado.id_fabricante
